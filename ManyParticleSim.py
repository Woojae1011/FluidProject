...
# Wall forces
wall_rep_force = 1 * force_const * obj_num / 100
...
keys_sorted, neiboring_particle_sorted, keys_unique, keys_unique_index, keys_unique_inv, keys_unique_count = [
...
def rep_force(i: int, j: int) -> None:
    ...
    rel_pos = pos[:, i] - pos[:, j]
    dit2 = max(rel_pos[0]**2 + rel_pos[1]**2, eps2)
    if dit2 < force_radius2:
        if power == 2:
            f = force_const * (rel_pos) / (dit2)
        elif power % 2 != 0:
            dist = max(np.linalg.norm(rel_pos), min_dist)
            f = force_const * (rel_pos) / (dist**power)
        else:
            f = force_const * (rel_pos) / (dit2**(power/2))
        force[:, i] += f
        force[:, j] -= f
...
def sim_update(dt: float) -> None:
    ...
    #Update map every map_update_freq
    # inter += 1
    # if inter >= map_update_freq:
    #     map_update()
    #     inter = 0
    ...
    for i, key in enumerate(keys_unique): # key = each occupied cell
        ...
        for neiboring_particle_keys in neiboring_cells: # lists of keys for occupied neighboring cells
            ...
            # Updating neighboring_particles
    ...
    # O(n**2) implementation of force (testing)
    # for i in range(obj_num):
    #     for j in range(obj_num):
    #         if i < j:
    #             old_force = force
    #             rep_force(particle1, particle2)
    #             print(old_force[:, j]-force[:, j])

    # rel_pos = pos[particle1] - pos[particle2] 
    # #   = (n-1 x 1) - (k x m) : k: num cells containing particle, m: (max) num particles in cell + neighbors
    # #   = pos of each particle - pos of particles in particle's cell + neighboring cells
    # rel_pos = n * k
    # dist2 = rel_pos**2
    # np.where(dist2 < force_radius2, np.linalg.norm(rel_pos), 0)
    ...
    #Wall collision
    if wall_bounce: # if wall collision in on
        vel[...] = vel * np.where(wall_contact != 0, -wall_bounce_coeff, 1)
    else:
        vel[...] = vel * np.where(wall_contact != 0, 0, 1) # Clamp velocity to 0 on wall contact
    ...
    # pos = pos_new
    # vel = vel_new
    acc[...] = acc_new #Reassign w/o new allocation
    ...
    # Color on speed
    # color[0] = (speed / max_speed * 0.7) ** 0.4
    # Color on map
...
def update(frame):
    ...
    global time_start, last_frame_time, fps, del_frames, dt
    ...
    # Compute FPS approximation using recent frame durations
    frame += 1  #round((frame_time - time_start) / sim_length)

    # Clamp dt to avoid instability under significant lag
    dt = max_dt # min(frame_time - last_frame_time, max_dt)
...
def main(length: float = 10.0, ani_save: bool = False, ani_show: bool = True) -> None:
    ...
    time_taken = timedelta(seconds=last_sim_seconds)
